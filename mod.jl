function build_energy_model(data_file::String)
    ...
    for hour in 2:length(time_arr)-1
        @constraint(m,batterystorage[1,hour] == batterystorage[1,hour-1] + sum(x[i,1,hour] for i in I) - Load_DE[s] ) #Battery charge flow DE
    end

    for hour in 2:length(time_arr)-1
        @constraint(m,batterystorage[2,hour] == batterystorage[2,hour-1] + sum(x[i,2,hour] for i in I) - Load_SE[s] ) #Battery charge flow SE
    end

    for hour in 2:length(time_arr)-1
        @constraint(m,batterystorage[3,hour] == batterystorage[3,hour-1] + sum(x[i,3,hour] for i in I) - Load_DK[s] ) #Battery charge flow DK
    end


    for country in J
        @constraint(m,batterystorage[country,1] == batterystorage[country,end] )
    end
